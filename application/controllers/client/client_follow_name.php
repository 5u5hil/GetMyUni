...
class Client_Follow_School extends CI_Controller
 {
	   ...
	   function follow_school_info()
    {
		...
		$json_array['error']										= 'success';
				...
				// echo $data['following_count'] = $this->model->following_count() ."_".;
				if(is_array($data['get_student_following_college'])){foreach ($data['get_student_following_college'] as $followers) {
                                                if ($count % 4 == 1) {
                                                    ?>
